...
def build_parser() -> argparse.ArgumentParser:
    ...
    def add_compute(s):
      s.add_argument("--config", required=True); s.add_argument("--output-dir", required=True); s.add_argument("--enable-global", action="store_true"); s.add_argument("--enable-seg", action="store_true"); s.add_argument("--enable-dvf", action="store_true"); s.add_argument("--enable-motion", action="store_true"); s.add_argument("--enable-vertebra", action="store_true"); s.add_argument("--use-gpu", action="store_true"); s.add_argument("--device", default="cuda:0"); s.add_argument("--gpu-metrics", default="all"); s.add_argument("--ncc-batch-size", type=int, default=64)
...
def main(argv=None) -> None:
    ...
    if args.cmd in {"compute","all"}:
      cfg=load_config(args.config); combined,_,_=compute_from_config(cfg,args.output_dir,args.enable_global,args.enable_seg,args.enable_dvf,args.enable_motion,args.enable_vertebra,args.use_gpu,args.device,args.gpu_metrics,args.ncc_batch_size)
