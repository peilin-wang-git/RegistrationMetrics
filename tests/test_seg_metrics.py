...
def test_selected_seg_organs_columns_and_heart_to_kidney_mean():
    ...
    out = compute_segmentation_metrics(fixed, moving, warped, label_map, (1, 1, 1), "case", 0, min_mask_volume_voxels=1)
...
def test_seg_empty_mask_returns_nan():
    fixed = np.zeros((4, 4, 4), dtype=int)
    moving = np.zeros_like(fixed)
    warped = np.zeros_like(fixed)
    fixed[1:3, 1:3, 1:3] = 17
    label_map = {0: "background", 17: "heart"}
    out = compute_segmentation_metrics(fixed, moving, warped, label_map, (1, 1, 1), "case", 0, min_mask_volume_voxels=1)
    for metric in ["dice", "iou", "hd95", "assd"]:
        assert np.isnan(out[f"{metric}_heart_warped_fixed"])
        assert np.isnan(out[f"{metric}_heart_moving_fixed"])


def test_seg_mean_ignores_nan_and_all_nan():
    fixed = np.zeros((4, 4, 4), dtype=int)
    moving = np.zeros_like(fixed)
    warped = np.zeros_like(fixed)
    fixed[0:2, 0:2, 0:2] = 17
    moving[0:2, 0:2, 0:2] = 17
    warped[0:2, 0:2, 0:2] = 17
    fixed[2:4, 2:4, 2:4] = 10
    label_map = {0: "background", 10: "gallbladder", 17: "heart"}
    out = compute_segmentation_metrics(fixed, moving, warped, label_map, (1, 1, 1), "case", 0, min_mask_volume_voxels=1)
    assert np.isclose(out["mean_dice_all_organs_moving_fixed"], 1.0)
